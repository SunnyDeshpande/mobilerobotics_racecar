...
class LaneDetector(Node):
    ...
    def image_callback(self, msg: Image):
        ...
        mask_gray = cv2.adaptiveThreshold(
            ...
            cv2.THRESH_BINARY, 151, self.adaptive_c #-20 for dark
        )

        white_pixel_count = np.sum(mask_gray == 255)
        ...
        lower_white = np.array([0, 0,  0], dtype=np.uint8) # 0, 40, 0 for dark
        upper_white = np.array([180, 255, self.adaptive_saturation], dtype=np.uint8)

        mask_hls = cv2.inRange(hls, lower_white, upper_white)

        white_pixel_count = np.sum(mask_hls == 255)
        total_pixels = mask_gray.size
        white_percentage = white_pixel_count / total_pixels
